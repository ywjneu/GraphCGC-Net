...
def compute(adj, base_adj):
    ...
    base_adj_ -= np.diag(np.diag(base_adj_))  # 将 adj_ 对角线上的元素置零
...
def show_graph(adj, base_adj, remove_isolated=True, args=None, i=0):
    # adj 是生成图的邻接矩阵，base_adj 是原始图的邻接矩阵
    # 绘制原始图的坐标图
    if not isinstance(base_adj, np.ndarray):
        base_adj_ = base_adj.data.cpu().numpy()
    else:
    ...
    # neato用来绘制无向图，layout的结果是根据force-directed algorithms也就是spring algorithms算出来的。
...
def show_graph1(adj, base_adj, remove_isolated=True, args=None, i=0):  # adj 是生成图的邻接矩阵，base_adj 是原始图的邻接矩阵
    ...
    base_adj_ -= np.diag(np.diag(base_adj_))  # 将 adj_ 对角线上的元素置零
    # base_G = nx.from_numpy_matrix(base_adj_)
    base_G = nx.MultiGraph(base_adj_)
    base_pos = graphviz_layout(base_G, prog='neato')
    # base_pos = nx.spring_layout(base_G)
    plt.figure(3, figsize=(3, 3))
    nx.draw(base_G, base_pos, **options)
    plt.savefig('./{}/{}_original_graph'.format(args.output_dir, i), dpi=500)
    plt.close()
    # 绘制原始图的坐标图
    if not isinstance(adj, np.ndarray):
        adj_ = adj.data.cpu().numpy()
    ...
    # assert ((adj_ == adj_.T).all())
    gen_G = nx.from_numpy_matrix(adj_)
    nx.draw(gen_G, base_pos, node_size=20, width=0.2, edge_color='grey', node_color='r')
    plt.savefig('./{}/{}_generated_graph'.format(args.output_dir, i), dpi=1000)
    plt.title("SuperNode")
    plt.close()
...
def normalize(adj):                                     # 邻接矩阵归一化处理，即 D^{-1/2}(A+I)D^{1/2}
	adj = adj.coarsened_data.cpu().numpy()
...
def topk_adj(adj, k):  # k // 2 是当前的训练邻接矩阵的边数，topk_adj 函数的作用是选出 adj 中最大的 k // 2 个元素（注，此时解码器输出的adj并不是二值矩阵）并置1，其余元素置零
    ...
    for temp in adj:
        ...
        adj_ = (adj_ - np.min(adj_)) / np.ptp(
            adj_)  # np.ptp(adj_) 计算 adj_ 中最大元素与最小元素的差，这一行的作用是将 adj_ 中的每个元素与最小元素的差除以 adj_ 中最大元素与最小元素的差
...
def gen_adj(model, n, e, z_size):  # gen_adj 的作用是生成具有 n 个节点，e 条边的邻接矩阵
    # model 是生成器模型，n 是训练邻接矩阵的节点数，e 是训练邻接矩阵的边数，attr_vec 是条件向量，z_size 是噪声向量的维度
